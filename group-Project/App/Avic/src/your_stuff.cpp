...
/*         if(_frame->data[7] == 113)
     ...
     case CAN::MSG::SHUTDOWN_ID: {
        std::cout<< "shutdown "<< (int)_frame->data[0]<< std::endl;
        //this->InstrumentCluster.setFuelGauges(_frame->data[1]);
        if(_frame->data[0] == 113)
        {
            QGuiApplication::quit();
        }
        this->InstrumentCluster.setFuelGauges(_frame->data[1]);
