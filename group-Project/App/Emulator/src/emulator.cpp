...
void Emulator::CalculateOilWaterTemperature(Decoded_data *_data, std::atomic<bool> *_exit_flag)
{
    while ((_data->oil_temperature < max_oil_temperature)&&(_data->water_temperature < max_water_temperature)&& (!_exit_flag->load()))
    {
        ...
        std::cout << "WATER kjgkjg TEMPERATURE = " << _data->water_temperature << std::endl;
    }
}
...
void Emulator::CalculateSpeed(Decoded_data *_data, const double _max_speed)
{
    ...
    _data->speed += (_data->decoded_throttle * throttle_ratio)/100.0;

    if(_data->speed > _max_speed) {
        _data->speed = _max_speed;
        _data->decoded_throttle=0;
    }
...
{
    if (_data->speed >= 0 && _data->speed <= 40)
    {
        _data->gear_num = 1;
    }
    else if (_data->speed > 40 && _data->speed <= 70)
    {
        _data->gear_num = 2;
    }
    else if (_data->speed > 70 && _data->speed <= 120)
    {
        _data->gear_num = 3;
    }
    else if (_data->speed > 120 && _data->speed <= 160)
    {
        _data->gear_num = 4;
    }
    else if (_data->speed > 160 && _data->speed <= 200)
    {
        _data->gear_num = 5;
    }
...
void Emulator::CalculateRPM(Decoded_data *_data)
{
    ...
    else if (_data->gear_num == 2 && _data->speed <= 70)
    {
        gear_num_ratio = 43;
    }
    else if (_data->gear_num == 3 && _data->speed <= 120)
    {
        gear_num_ratio = 30;
    }
    else if (_data->gear_num == 4 && _data->speed <= 160)
    {
        gear_num_ratio = 23;
    }
    else if (_data->gear_num == 5 && _data->speed <= 200)
    {
        gear_num_ratio = 23;
    }
